...
if(!class_exists('AQ_Page_Builder')) {
	class AQ_Page_Builder {
		...
		function admin_enqueue() {
			...
			// Custom CSS
			wp_register_style( 'custom-css', $this->url.'assets/css/custom.css', array(), time(), 'all');
		...
		**/
		function display_template($template_id) {
			...
			if(empty($blocks)) {
				...
				$overgrid = 0; $span = 0; $first = false; $last = false;
				...
				foreach($blocks as $key => $instance) {
					...
					if(class_exists($id_base)) {
						...
						if($parent == 0) {
							...
							if($overgrid > 12 || $span == 12 || $span == 0) { // first column
								if($span == 0) $instance['veryfirst'] = true; // (foundation grid)
							...
							$overgrid = 0; $first = false; $last = false; // reset vars
						}
					}
				}
				
				//close template wrapper
				echo '</div>'; // close last row (foundation grid)
				echo '</div>';
			}
